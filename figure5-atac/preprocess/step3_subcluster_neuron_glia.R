...
# for(PROJDIR in c('Mouse_scATAC_DorsalHorn_neuron','Mouse_scATAC_DorsalHorn_glia', 'Mouse_scATAC_DorsalHorn_neuron2')){

for(PROJDIR in c('Mouse_scATAC_DorsalHorn_glia')){

  proj = loadArchRProject(path = here::here('data/tidy_data/ArchRProjects', PROJDIR))
  varFeats = c(10,20,30,40,50)
...
#   if (ClustersName2 %ni% names(pd)){
    ...
    proj <- addClusters( proj, reducedDims = HarmonyName, 
                         ...
                         filterBias = TRUE,resolution = 2, maxClusters = 25, force = TRUE)
