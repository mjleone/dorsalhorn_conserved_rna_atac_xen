...
PathToNarrowPeak="/projects/pfenninggroup/singleCell/BICCN_mouse_CATlas_snATAC-seq/data/raw_data/halper/*Mus_musculusToHomo_sapiens.HALPER.narrowPeak.gz"
...
zcat ${PathToNarrowPeak} | awk 'OFS="\t" {print $1, $2, $3}'| bedtools sort -i stdin | bedtools merge -i stdin > ${PathToOutput}
gzip ${PathToOutput}
...
PathToOutput=${EnrichmentsDir}"backgroundBedFiles/DHS_all_cell_types.bed"
...
sbatch --partition pool1 /projects/pfenninggroup/machineLearningForComputationalBiology/gwasEnrichments/scripts/annotate_bed_LDSC_hg38.sh -i ${PathToOutput} -n DHS_all_cell_types -o ${PathToAnnotOutput}
